<script>
  ...
  async function fetchManifest() {
    try {
      const res = await fetch(`/api/manifest/Test_APM`);
      const manifest = await res.json();

      if (manifest?.main?.dish_generate_reply?.routing_key) {
        chatCommandKey = manifest.main.dish_generate_reply.routing_key + ".0";
      } else {
        console.error("❌ No dish_generate_reply key found in manifest:", manifest);
        chatCommandKey = "Test_APM.main.dish_generate_reply.0"; // fallback
      }
  ...
  async function sendMessage() {
    ...
    const url = '/api/modules/Test_APM/dish/respond?stream=true';
